...
using POS.TransactionFile.TransferIn;
using System;
using System.Data;
...
namespace POS.Item
{
    class TransferInOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\TransferInInvoice.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\TransferInInvoice.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\TransferInInvoice.xls";
        DataSet1 ds;
        public TransferInOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            ReportClass rp; 
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new TransferInOrderReportsPashtu();
            else
                rp = new TransferInOrderReports();
