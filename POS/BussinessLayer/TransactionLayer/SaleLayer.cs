...
namespace MDS.Customer
{
    class SaleLayer
    {
        ...
        internal int SaveUpdateFnc(string txtcode, SaleOrderModel model)
        {
            int result = 0;
            if (txtcode == string.Empty)
            {
                db.SaleOrderModels.Add(model);
                result = db.SaveChanges();

            }
            else
            {
                model.Sale_Order_ID = Convert.ToInt32(txtcode);
                var ItemINDB = db.SaleOrderModels.Find(model.Sale_Order_ID);
        ...
        {
            return db.SaleOrderModels.Max(x => x.Sale_Order_ID);
        }
        internal int SaveUpdateFncInvoice(SaleInvoiceModel model, int maincode)
        {
            int result = 0;
            model.Sale_Order_ID = maincode;
            db.SaleInvoiceModels.Add(model);
            result = db.SaveChanges();
        ...
        internal int DeleteFnc(int txtcode)
        {
            ...
            try
            {
                SaleOrderModel model = db.SaleOrderModels.Find(txtcode);
                db.SaleOrderModels.Remove(model);
                result = db.SaveChanges();
            }
            catch (Exception)
            {
        ...
        internal DataTable LoadGridView(string state)
        {
            ...
            var mod = db.SaleOrderModels.Include(u => u.UserModels)
                .Include(u => u.CustomerModels).ToList().OrderByDescending(x => x.Sale_Order_ID);
            ...
            foreach (var item in mod)
            {
                ...
                row[8] = item.Cu_ID;
                var account = db.SaleAccountModels.Where(u => u.Sale_Order_ID == item.Sale_Order_ID).ToList();
        ...
        internal List<SaleInvoiceTempList> LoadInvoiceDataGrid(int iD)
        {
            List<SaleInvoiceTempList> li = new List<SaleInvoiceTempList>();


            var tempModel = db.SaleInvoiceModels.Include(u => u.itemModel)
                    .Where(u => u.Sale_Order_ID == iD).ToList().OrderByDescending(u => u.ID);

            foreach (var item in tempModel)
            {
                li.Add(new SaleInvoiceTempList()
                {
        ...
        internal void DeleteInvoiceAndAccount(int code)
        {
            try
            {
                db.SaleInvoiceModels.RemoveRange(db.SaleInvoiceModels.Where(u => u.Sale_Order_ID == code));
                db.SaleAccountModels.RemoveRange(db.SaleAccountModels.Where(u => u.Sale_Order_ID == code));
                db.SaveChanges();
            }
            catch (Exception)
            {
