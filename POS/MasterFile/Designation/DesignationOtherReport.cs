...
namespace POS.Item
{
    class DesignationOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\DesignationList.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\DesignationList.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\DesignationList.xls";
        DataSet1 ds;
        ReportClass rp;
        public DesignationOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new DesignationReportpastu();
            else
                rp = new DesignationReport();
