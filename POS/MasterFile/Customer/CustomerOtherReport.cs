...
namespace POS.Item
{
    class CustomerOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\customer.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\customer.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\customer.xls";
        DataSet1 ds;
        public CustomerOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            ReportClass rp; 
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new CustomerReportPashtu();
            else
                rp = new CustomerReport();
