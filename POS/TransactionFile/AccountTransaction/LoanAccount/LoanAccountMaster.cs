...
namespace POS.Item
{
    public partial class LoanAccountMaster : Form
    {
        private LoanAccountLayer layer = new LoanAccountLayer();
        ...
        private LoanAccountMain owner;
        private LoanAccountOrderModel Model;

        private string saveupdate { get; set; }

        public LoanAccountMaster(LoanAccountMain _owner, string save)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (LoanAccountMain)_owner;
            txtdate.Value = DateTime.Now;
            txtcode.Select();
        }

        public LoanAccountMaster(LoanAccountMain _owner, string save, LoanAccountOrderModel model)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (LoanAccountMain)_owner;
            Model = (LoanAccountOrderModel)model;
        ...
        private void ConvertLanguage()
        {
            ...
            lblreceived.Text = Language.recieved;
            lblreceived.Font = Language.font;
            ...
            lbltitle.Text = "څانګې یا د پور حساب";
        ...
        private LoanAccountOrderModel SetProperty()
        {
            LoanAccountOrderModel model = new LoanAccountOrderModel();
        ...
        private TransferInAccountModel SetInvoiceProperty(int i)
        {
            TransferInAccountModel model = new TransferInAccountModel();
            model.lo_id = Convert.ToInt32(bunifuCustomDataGrid1.Rows[i].Cells["Code"].Value.ToString());
            ...
            if (bunifuCustomDataGrid1.RowCount != 1)
                for (int i = 0; i < bunifuCustomDataGrid1.RowCount - 1; i++)
                {
                    ...
                    if (txtcode.Text == bunifuCustomDataGrid1.Rows[i].Cells["Code"].Value.ToString())
                    {
                        ...
                        txtcode.Focus();
                        MessageBox.Show(txtcode.Text + " Name already exist");
                        return false;
                    }
                }
        ...
        internal void LoadDataForprint(string fromform,string p)
        {
            ...
            switch (p)
            {
                ...
                case "Preview":
                    LoanAccountReportForm obj = new LoanAccountReportForm(ds);
                    obj.Show();
                    break;
                case "Print":
                    LoanAccountOtherReport obj2 = new LoanAccountOtherReport(p, ds);
                    break;
                case "Word":
                    LoanAccountOtherReport obj3 = new LoanAccountOtherReport(p, ds);
                    break;
                case "Excel":
                    LoanAccountOtherReport obj4 = new LoanAccountOtherReport(p, ds);
                    break;
