...
using POS.Reports.SupplierLedger;
using System;
using System.Data;
...
namespace POS.Item
{
    class SupplierLedgerOtherReport
    {
        readonly DateTime fromDate;
        readonly DateTime toDate;
        readonly decimal debit;
        readonly decimal credit;
        readonly DataSet1 ds;

        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\SupplierLedger.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\SupplierLedger.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\SupplierLedger.xls";
        ReportClass rp;
        public SupplierLedgerOtherReport(string p,
            ...
            decimal credit, decimal debit, DataSet s)
        {
            ...
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new SupplierLedgerReportPashtu();
            else
                rp = new SupplierLedgerReport();
         

            rp.SetDataSource(ds.Tables["tbl_CustomerLedger"]);
            ...
            rp.SetParameterValue("@debit", debit);
            rp.SetParameterValue("@credit", credit);
