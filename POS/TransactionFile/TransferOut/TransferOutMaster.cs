...
namespace POS.Item
{
    public partial class TransferOutMaster : Form
    {
        private TransferOutLayer layer = new TransferOutLayer();
        ...
        private TransferOutMain owner;
        private TransferOutOrderModel Model;
        ...
        public TransferOutMaster(TransferOutMain _owner, string save)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (TransferOutMain)_owner;
            txtdate.Value = DateTime.Now;
            txtcode.Select();
        }

        public TransferOutMaster(TransferOutMain _owner, string save, TransferOutOrderModel model,AccountTempModel account)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (TransferOutMain)_owner;
            Model = (TransferOutOrderModel)model;
        ...
        {
            txtcode.Text = Model.LoanModels.Lo_ID.ToString();
            txtname.Text = Model.LoanModels.Name;
            txtaddress.Text = Model.LoanModels.Address1;
            txtmaincode.Text = Model.Transfer_Out_ID.ToString();
            ...
            List<TransferOutInvoiceTempList> li = layer.LoadInvoiceDataGrid(Model.Transfer_Out_ID);
            foreach (var item in li)
            {
                bunifuCustomDataGrid1.Rows.Add( bunifuCustomDataGrid1.RowCount,
                         ...
                         item.Price.ToString(),
                         item.Bonus.ToString(),
                         item.Disc.ToString()),item.Tp_Price.ToString());
            }
        ...
        private void ConvertLanguage()
        {
            ...
            lblphone.Text = Language.InvoiceNumber;
            ...
            lbltitle.Text = "بهر د بیل انتقال";
        ...
        private TransferOutOrderModel SetProperty()
        {
            TransferOutOrderModel model = new TransferOutOrderModel();
            model.Lo_ID = Convert.ToInt32(txtcode.Text);
        ...
        private TransferOutInvoiceModel SetInvoiceProperty(int i)
        {
            TransferOutInvoiceModel model = new TransferOutInvoiceModel();
            ...
            model.Tp_Price = Convert.ToDecimal(bunifuCustomDataGrid1.Rows[i].Cells[8].Value.ToString());
            return model;


        }
        private TransferInAccountModel SetAccountProperty(int txtmaincode,string txtcode,DateTime date,string type,decimal amount,decimal recieved )
        {
            TransferInAccountModel model = new TransferInAccountModel();
            model.Transfer_In_ID = 0;
            model.Transfer_Out_ID = txtmaincode;
        ...
        private void loadSearchFormSupplier()
        {
            ...
            if (obj.loanmodel != null)
            {
                ...
                txtinvoice.Text = obj.loanmodel.phone1.ToString();
        ...
        public decimal gettpprice { get; set; }
        private void loadSearchFormItem()
        {
            ...
            if (obj.ItemModel != null)
            {
                ...
                gettpprice = (decimal)obj.ItemModel.item_tp;
                //MessageBox.Show(tpprice.ToString());
            ...
            switch (p)
            {
                ...
                case "Preview":
                   TransferOutReportForm obj = new TransferOutReportForm(ds);
                    ...
                    break;
                case "Print":
                    TransferOutOtherReport obj2 = new TransferOutOtherReport(p, ds);
                    break;
                case "Word":
                    TransferOutOtherReport obj3 = new TransferOutOtherReport(p, ds);
                    break;
                case "Excel":
                    TransferOutOtherReport obj4 = new TransferOutOtherReport(p, ds);
                    break;
