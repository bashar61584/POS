...
namespace POS.Item
{
    public partial class CustomerAccountMaster : Form
    {
        private CustomerAccountLayer layer = new CustomerAccountLayer();
        ...
        private CustomerAccountMain owner;
        private CustomerAccountOrderModel Model;

        private string saveupdate { get; set; }

        public CustomerAccountMaster(CustomerAccountMain _owner, string save)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (CustomerAccountMain)_owner;
            txtdate.Value = DateTime.Now;
            txtcode.Select();
        }

        public CustomerAccountMaster(CustomerAccountMain _owner, string save, CustomerAccountOrderModel model)
        {
            InitializeComponent();
            saveupdate = save;
            owner = (CustomerAccountMain)_owner;
            Model = (CustomerAccountOrderModel)model;
        ...
        private void GetProperty()
        {
            ...
            txtnetamount.Text = Model.Credit.ToString();
            txtrecieved.Text = Model.Debit.ToString();
        ...
        private void ConvertLanguage()
        {
            ...
            lblitemreceived.Text = Language.recieved;
            lblitemreceived.Font = Language.font;
            lblitempayed.Text = Language.payed;
            lblitempayed.Font = Language.font;
            lbluser.Text = Language.user;
            lblphone.Text = Language.Phone1;          
            btnaddlist.Text = Language.add;
            btndeletelist.Text = Language.delete;
            lbltitle.Text = "د پیرودونکی حساب";
            btnSave.Text = Language.save;
            btnDelete.Text = Language.delete;
            lbltotalreceived.Text = Language.recieved;
            lbltotalpayed.Text = Language.payed;

        }

        private CustomerAccountOrderModel SetProperty()
        {
            CustomerAccountOrderModel model = new CustomerAccountOrderModel();
        ...
        private SaleAccountModel SetInvoiceProperty(int i)
        {
            SaleAccountModel model = new SaleAccountModel();
            model.Cu_ID = Convert.ToInt32(bunifuCustomDataGrid1.Rows[i].Cells["Code"].Value.ToString());
        ...
        internal void LoadDataForprint(string fromform,string p)
        {
            ...
            switch (p)
            {
                ...
                case "Preview":
                    CustomerAccountReportForm obj = new CustomerAccountReportForm(ds);
                    obj.Show();
                    break;
                case "Print":
                    CustomerAccountOtherReport obj2 = new CustomerAccountOtherReport(p, ds);
                    break;
                case "Word":
                    CustomerAccountOtherReport obj3 = new CustomerAccountOtherReport(p, ds);
                    break;
                case "Excel":
                    CustomerAccountOtherReport obj4 = new CustomerAccountOtherReport(p, ds);
                    break;
