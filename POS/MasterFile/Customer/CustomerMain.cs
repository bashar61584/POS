...
namespace POS.Item
{
    ...
    {
        readonly CustomerLayer layer = new CustomerLayer();
        ...
        public CustomerMain(string dec)
        {
            InitializeComponent();
            txtsearch.Select();
            this.decession = dec;
        ...
        {
            if (POS.Properties.Settings.Default.customerlist)
            {
                CustomerMaster item = new CustomerMaster(this, "Save");
                item.ShowDialog();
            }
            else
            {
        ...
        private void BtnOpen_Click(object sender, EventArgs e)
        {
            if (POS.Properties.Settings.Default.customerlist)
            {
                if (bunifuCustomDataGrid1.RowCount != 1)
                    ...
                    {
                        CustomerMaster item = new CustomerMaster(this, "Update", SetProperty());
                        item.ShowDialog();
                    }
            }
            else
        ...
        private void ConvertLanguage()
        {
            ...
            lbltitle.Text = "د پیرودونکی لیست";
            this.Text = "د پیرودونکی لیست";
        } 
        ...
        private CustomerModel SetProperty()
        {
            CustomerModel model = new CustomerModel();
            
            
            model.Cu_ID = Convert.ToInt32(bunifuCustomDataGrid1.SelectedRows[0].Cells[0].Value.ToString());
