...
namespace POS.Item
{
    class ItemOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\ItemList.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\ItemList.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\ItemList.xls";
        DataSet1 ds;
        ReportClass rp;
        public ItemOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new ItemReportpastu();
            else
                rp = new ItemReport();
