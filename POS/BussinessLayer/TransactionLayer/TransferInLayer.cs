...
{
    class TransferInLayer
    {
        private ModelContext db;
        public TransferInLayer()
        {
            db = new ModelContext();

        }

        internal int SaveUpdateFnc(string txtcode, TransferInOrderModel model)
        {
            int result = 0;
            if (txtcode == string.Empty)
            {
                db.TransferInOrderModels.Add(model);
                result = db.SaveChanges();

            }
            else
            {
                model.Transfer_In_ID = Convert.ToInt32(txtcode);
                var ItemINDB = db.TransferInOrderModels.Find(model.Transfer_In_ID);
        ...
        {
            return db.TransferInOrderModels.Max(x => x.Transfer_In_ID);
        }
        internal int SaveUpdateFncInvoice(TransferInInvoiceModel model, int maincode)
        {
            int result = 0;
            model.Transfer_In_ID = maincode;
            db.TransferInInvoiceModels.Add(model);
            result = db.SaveChanges();
        ...
        internal int DeleteFnc(int txtcode)
        {
            ...
            try
            {
                TransferInOrderModel model = db.TransferInOrderModels.Find(txtcode);
                db.TransferInOrderModels.Remove(model);
                result = db.SaveChanges();
            }
            catch (Exception)
            {
        ...
        internal DataTable LoadGridView(string state)
        {
            ...
            var mod = db.TransferInOrderModels.Include(u => u.UserModels)
                .Include(u => u.LoanModels).ToList().OrderByDescending(x => x.Transfer_In_ID);
            ...
            foreach (var item in mod)
            {
                ...
                var account = db.TransferInAccountModels.Where(u => u.Transfer_In_ID == item.Transfer_In_ID).ToList();
        ...
        internal List<TransferInInvoiceTempList> LoadInvoiceDataGrid(int iD)
        {
            List<TransferInInvoiceTempList> li = new List<TransferInInvoiceTempList>();


            var tempModel = db.TransferInInvoiceModels.Include(u => u.itemModel)
                    .Where(u => u.Transfer_In_ID == iD).ToList().OrderByDescending(u => u.ID);

            foreach (var item in tempModel)
            {
                li.Add(new TransferInInvoiceTempList()
                {
        ...
        internal void RetrieveReport(string p)
        {
            ...
            switch (p)
            {
                ...
                case "Preview":
                    ...
                    break;
                case "Print":
                    TransferInOtherReport orp = new TransferInOtherReport(p, ds);

                    break;
                case "Word":
                    TransferInOtherReport orp2 = new TransferInOtherReport(p, ds);

                    break;
                case "Excel":
                    TransferInOtherReport orp3 = new TransferInOtherReport(p, ds);
        ...
        internal void DeleteInvoiceAndAccount(int code)
        {
            try
            {
                db.TransferInInvoiceModels.RemoveRange(db.TransferInInvoiceModels.Where(u => u.Transfer_In_ID == code));
                db.TransferInAccountModels.RemoveRange(db.TransferInAccountModels.Where(u => u.Transfer_In_ID == code));
                db.SaveChanges();
            }
            catch (Exception)
            {
