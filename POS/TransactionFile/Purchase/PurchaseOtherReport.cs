...
namespace POS.Item
{
    class PurchaseOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseInvoice.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseInvoice.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseInvoice.xls";
        DataSet1 ds;
        public PurchaseOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            ReportClass rp;
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new PurchaseOrderReportsPashtu();
            else
                rp = new PurchaseOrderReports();
