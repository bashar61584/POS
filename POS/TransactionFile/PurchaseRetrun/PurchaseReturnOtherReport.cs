...
using POS.TransactionFile.PurchaseRetrun;
using System;
...
namespace POS.Item
{
    class PurchaseReturnOtherReport
    {
        private string FilePath = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseReturnInvoice.doc";
        private string FilePathEmail = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseReturnInvoice.pdf";
        private string FilePathExcel = @"C:\users\" + Environment.UserName.ToString() + @"\documents\PurchaseReturnInvoice.xls";
        DataSet1 ds;
        public PurchaseReturnOtherReport(string p,DataSet s)
        {
            ds = (DataSet1)s;
            ReportClass rp;
            if (POS.Properties.Settings.Default.Language == 1)
                rp = new PurchaseReturnReportsPashtu();
            else
                rp = new PurchaseReturnReports();
