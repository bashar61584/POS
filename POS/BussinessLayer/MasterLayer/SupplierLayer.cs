...
namespace MDS.Customer
{
    class SupplierLayer
    {
        ...
        internal int SaveUpdateFnc(string txtcode, SupplierModel model )
        {
            int result = 0;
            if (txtcode==string.Empty)
            {
                db.SupplierModels.Add(model);
            ...
            else
            {
                ...
                var ItemINDB = db.SupplierModels.Find(model.Sup_ID);
        ...
        internal int DeleteFnc(string txtcode)
        {
            ...
            try
            {
                SupplierModel model = db.SupplierModels.Find(Convert.ToInt32(txtcode));
                db.SupplierModels.Remove(model);
                result = db.SaveChanges();
            }
            catch (Exception)
            {
        ...
        internal DataTable LoadGridView(string state)
        {
            ...
            var mod = db.SupplierModels.Include(u => u.UserModels).ToList().OrderByDescending(x => x.Sup_ID);
        ...
        internal void RetrieveReport(string p)
        {
            ...
            switch (p)
            {
                ...
                case "Preview":
                    SupplierReportForm obj = new SupplierReportForm(ds);
                    obj.Show();
                    break;
                case "Print":
